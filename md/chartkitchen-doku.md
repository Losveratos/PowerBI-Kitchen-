...
> Ein IBCS-inspiriertes Chart-Deck fuer Power BI: 13 Diagramm-Modi in einem einzigen Custom Visual, mit einheitlicher Szenario-Notation (AC ausgefuellt, PY grau, PL Umriss, FC schraffiert).

- **Quelle:** https://datenwgknowledgekitchen.com/chartkitchen-doku.html
- **Autor:** Michael Tenner · Daten-WG Knowledge Kitchen
- **Extrahiert aus:** `chartkitchen-doku.html` · Stand 2026-07-19 (Git-Commit-Datum der Quelldatei)
- **Zitierhinweis:** Michael Tenner, Daten-WG Knowledge Kitchen, https://datenwgknowledgekitchen.com/chartkitchen-doku.html — Abruf mit Datum angeben. Weiterverwendung mit Quellenangabe erwuenscht.
...
- **Englische Fassung:** https://datenwgknowledgekitchen.com/chartkitchen-doku_en.html
- **Schnellstart:** https://datenwgknowledgekitchen.com/chartkitchen-schnellstart.html
...
1. Was ist ChartKitchen?

2. Schnellstart

3. Die 13 Modi

4. ChartKitchen im Einsatz

5. Funktionen im Detail

6. Einstellungs-Referenz
...
Startseite bei leeren Feldern: Kachel-Übersicht aller Modi mit den je Modus benötigten Feldern.
...
Säulen (Zeitreihe): AC gegen PY als überlagerte Säulen, darüber die Varianz-Panels ΔPY absolut und ΔPY in Prozent.

So sieht es in Power BI Desktop ausSäulen-Modus mit vier gestapelten Varianz-Panels — ΔPL %, ΔPL, ΔPY % und ΔPY — über den AC-Säulen; die Forecast-Monate ab Juli sind schraffiert.
...
Balken (Struktur): Produkte nach AC absteigend, mit seitlichen ΔPY- und ΔPY-%-Panels zur Abweichungslesung.
...
Linie (viele Datenpunkte): AC solide, PY dünn und grau, plus gleitender 6-Perioden-Durchschnitt für den Trend.

So sieht es in Power BI Desktop ausLinien-Modus über das Geschäftsjahr: AC durchgezogen, PL gestrichelt und der Forecast-Abschnitt abgesetzt, darüber die Panels ΔPL und ΔPL % je Monat.
...
Wasserfall: GuV-Kaskade von Umsatz zu EBIT, Summenzeilen als volle Säulen, Delta-Zeilen als schwebende Brückenglieder.
...
Integrierte Brücke: PY-Totalsäule links, ΔPY-Kaskade mit Monatssäulen, gestapelte AC+FC-Totalsäule rechts.
...
Kategorie-Brücke: PY-Zeile oben, je Treiber ein AC-PY-Balken plus Kaskadenglied und ΔPY-%-Pin, AC-Zeile als Überleitung unten.

So sieht es in Power BI Desktop ausStruktur-Brücke nach Ländern: PL- und PY-Referenzzeile oben, ein AC-Balken je Land, rechts die ΔPL-Kaskade mit ΔPL-%-Pins und die AC-Summe unten.
...
Tabelle (IBCS): flache Wertetabelle mit AC-PY-Balken, ΔPL-Balkenspalte, ΔPL-%-Pins und Σ-Total-Zeile.

So sieht es in Power BI Desktop ausTabelle nach Ländern: AC-PY-PL-Balken je Zeile, eine eigene ΔPL-Balkenspalte, ΔPL-%-Pins und die Σ-Gesamt-Zeile unten.
...
Matrix: Zeilen-Kategorien gegen die Spaltenhierarchie Quartal→Monat, zugeklappt zu Quartals-Blöcken mit Σ, ΔPY und ΔPY-% rechts.

So sieht es in Power BI Desktop ausMatrix mit der Hierarchie Kategorie→Produkt und gleich vier Abweichungsspalten — ΔPL, ΔPL %, ΔPY und ΔPY % — je mit Balken bzw. Pins, plus Σ-Gesamt.
...
Pareto: AC-Säulen absteigend mit kumulierter Prozentlinie und 80-%-Referenzmarke für die ABC-Analyse.
...
Dumbbell: je Kategorie PY- und AC-Punkt, der farbige Verbinder zeigt Richtung und Größe der Veränderung.

So sieht es in Power BI Desktop ausDumbbell als Small Multiples je Land: je Produkt ein PL- und ein AC-Punkt, der Verbinder zeigt die Veränderung PL→AC.
...
Slope: Rangwechsel zwischen zwei Zeitpunkten, PY links und AC rechts, Steigung und Farbe je Kategorie.

So sieht es in Power BI Desktop ausSlope-Modus je Produkt: PY links, AC rechts, jede Linie ein Land — Steigung und Farbe zeigen Auf- und Absteiger auf einen Blick.
...
KPI-Karten: je Kennzahl eine Kachel mit AC-Wert, ΔPL- und ΔPY-Zeilen sowie einer Mini-Brücke PL→Δ→AC.
...
GuV-Statement: Ergebnisrechnung mit Summen-, Delta- und Margen-Zeilen, PY-/AC-Kaskadenspalten und ΔPY plus ΔPY-%-Pins.

So sieht es in Power BI Desktop ausGuV-Statement für zwei Gesellschaften nebeneinander: Zeilentypen Summe, Delta und Marge (Pp), die AC&FC-Kaskadenspalte sowie ΔPY und ΔPY %.
...
Eine komplette Berichtsseite: KPI-Karten, eine AC/FC-Monatsbrücke und eine YTD-Brücke nach Produkt — alle Kacheln aus demselben Visual, mit einheitlicher Szenario-Notation.

Integrierte Brücken je Kategorie oben, darunter dieselbe Brücke als Small Multiples je Land — so wird der Beitrag jeder Region zur Gesamtabweichung vergleichbar.

Frei gewählte Farbpalette: dieselbe integrierte Brücke mit Monatssäulen in Blau statt der IBCS-Graustufen — Szenario-Logik und Forecast-Schraffur bleiben erhalten.

Balken-Vergleich: per Klick werden zwei Länder markiert und direkt gegenübergestellt, während die ΔPL- und ΔPL-%-Panels rechts die Abweichung mitführen.

Top N mit Sammelposten: je Land eine KPI-Karten-Brücke, die kleineren Treiber zu „Rest (4)“ zusammengefasst — die Summe bleibt vollständig.

Year-to-go-Sicht: Restjahres-Bedarf je Land gegen Plan und Benchmark, rechts die PL→AC-Kaskade mit dem stärksten und schwächsten Treiber in der Botschaftszeile.

## 05Funktionen im Detail

### Szenarien, Forecast-Schraffur & Benchmark

Vier Szenarien: AC, PY, PL und FC in einem Chart, Forecast schraffiert, Benchmark-Marker als Dreieck und Ziel-Referenzlinie.
...
Small Multiples: eine Kachel je Region auf gemeinsamer Achse, jede mit eigenen Varianz-Panels und Zoom-Symbol.

So sieht es in Power BI Desktop ausTrellis nach Regionen (Americas · EMEA · APAC): je Region die Top-Länder als KPI-Karten mit Bullet-Balken, der Rest zu „Rest (N)“ gebündelt, jede Gruppe eigenständig sortierbar.
...
Breite Matrix im schmalen Visual: der Blockstreifen wird horizontal scrollbar, Namensspalte links und Σ-Block rechts bleiben fixiert.

So sieht es in Power BI Desktop ausBreite Matrix in Aktion: AC nach Monat gegen die Spaltengruppen Kategorie→Land; Namensspalte links und Σ-Spalte rechts bleiben fixiert, der Blockstreifen scrollt horizontal.
...
Zwei-Zeilen-Kompaktmodus (cellLayout stacked): Wert oben, kleiner ΔPY darunter — so passen mehr Spalten ohne Scrollen.
...
Werte-Spalten „all“: je Block die bezifferten Referenzwerte AC · PY · PL neben der ΔPL-Balkenspalte und dem Σ-Block.
...
Σ-Zeile oben (totalRowPosition top): die Summe steht unter dem Kopf, die Einzelzeilen folgen darunter.
...
Lesekomfort: Zebra-Streifen und Gitterlinien plus aufgeklappte Hierarchie Region→Land mit Einrückung und Chevrons.
...
Kommentare: nummerierte Marker ① ② an den betroffenen Säulen und die zugehörige Kommentarliste rechts neben dem Chart.

So sieht es in Power BI Desktop ausKommentar-Modus: nummerierte Marker ①–④ sitzen an den betroffenen Balken, die zugehörigen Notizen stehen als Liste rechts neben dem Chart.
...
Finanz-Format: Negativwerte in Klammern statt mit Minus, kombiniert mit den GuV-Zeilentypen Summe, Delta und Marge.
...
IBCS-Titel und Fußzeilen: „KPI · Einheit · Zeitraum: Szenarien“ mit Botschaftszeile oben, Filterkontext und Datenstand als Fußzeilen unten.
...
Monitoring-Karten: Statusstreifen und Hintergrund-Ampel bewerten AC gegen den Benchmark, ΔBM-Zeile und Bullet-Mikrografik je Kachel.

So sieht es in Power BI Desktop ausMonitoring-Karten je Land: der farbige Statusstreifen links bewertet AC gegen den Benchmark, darunter der ΔPY-Wert absolut und in Prozent.
...
Schema: Formatbereich: die sieben Karten des Visuals in Pane-Reihenfolge (zugeklappt) – IBCS-Titel, Chart, IBCS-Farben, Datenbeschriftungen, Kommentare, Skala & Referenzlinie, Kategorienachse.

### IBCS-Titel

Schema: Formatbereich → IBCS-Titel: Titel-Anzeige, KPI-Name, Zeitraum, Botschafts-Zeile, automatische Botschaft, Fußzeile und Filter-Fußzeile.
...
Schema: Formatbereich → Chart → Layout: Ausrichtung, Abweichungsbasis und die Abweichungs-Optionen.
...
Schema: Formatbereich → Chart → Analyse: Kumulierung (YTD), gleitender Durchschnitt, Top N, Hervorhebung, Invertierung, Wesentlichkeit und Pin-Form.
...
Schema: Formatbereich → Chart → Small Multiples: Gesamt-Kachel, Top N Kacheln, erste Kachel groß und gemeinsame Brücken-Skala.
...
Schema: Formatbereich → Chart → Brücke: Wasserfall-Brücke, Sortierung nach Wirkung, In-Chart-Buttons und Treiber-Notiz.
...
Schema: Formatbereich → Chart → Tabelle: Werte-Spalten, Struktur-Listen (Ergebnis-, Skip-, Ausblend-, Grafik-, Einrück-Zeilen), Formelzeilen sowie Zell-Layout, Σ-Position, Zebra, Zeilenhöhe und Gitterlinien.
...
Schema: Formatbereich → Chart → KPI-Karten: Status-Basis, Highlight-Status, Sortierung, Mini-Brücke, Hintergrund-Ampel und Bullet-Optionen.
...
Schema: Formatbereich → IBCS-Farben: Theme-Übernahme sowie die Szenario-Farben – Ist (AC) dunkelgrau, Vorjahr (PY) hellgrau, Plan (PL) als Umriss, gute Abweichung Teal, schlechte Abweichung Rot.
...
Schema: Formatbereich → Datenbeschriftungen: Anzeige, Dichte, Größen-Preset, Skalierung, Textgröße, Dezimalstellen, Einheiten, Finance-Format und Σ-treue Rundung.
...
Schema: Formatbereich → Kommentare: Kommentarliste anzeigen, Kommentar-Schriftgröße und Kommentar-Erfassung im Chart.
...
Schema: Formatbereich → Skala & Referenzlinie: Untergruppe Skalen-Sync (Mindest-Maxima, Ausreißer kappen) und Untergruppe Referenzlinie (Wert & Beschriftung).
...
Schema: Formatbereich → Kategorienachse: Textgröße der Achsenbeschriftung.
